...
class BiLSTMCRF(nn.Module):
    def __init__(self, sent_vocab, tag_vocab, dropout_rate=0.5, embed_size=256, hidden_size=256):
        ...
        # sent_vocab = {word: i for i, word in enumerate(tokenizer.get_vocab().keys())}
        
        
        self.dropout = nn.Dropout(dropout_rate)
        print('embed_size',embed_size)
        print('self.bert.config.hidden_size',self.bert.config.hidden_size)
        # self.encoder = nn.LSTM(input_size=embed_size, hidden_size=hidden_size, bidirectional=True)
        self.encoder = nn.LSTM(input_size=self.bert.config.hidden_size, hidden_size=hidden_size, bidirectional=True)
        self.hidden2emit_score = nn.Linear(hidden_size * 2, len(self.tag_vocab))
        print(len(self.tag_vocab))
    ...
    def forward(self, sentences, tags, sen_lengths):
        ...
        sentences = self.bert(sentences)[0]  # shape: (len, b, e)
        # print('bert',sentences.shape)
...
def main():
    ...
    print(list(sent_vocab.items())[:1000])
