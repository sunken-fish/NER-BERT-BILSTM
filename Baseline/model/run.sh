...
then
  python run.py test ./data/test.txt ./result.txt ./vocab/sent_vocab.json ./vocab/tag_vocab.json ./model.pth --cuda
  perl conlleval.pl < result.txt
elif [ "$1" = "test-without-cuda" ]
then
  python run.py test ./data/test.txt ./result.txt  ./vocab/sent_vocab.json ./vocab/tag_vocab.json ./model.pth
  perl conlleval.pl < result.txt
elif [ "$1" = "vocab" ]
then
